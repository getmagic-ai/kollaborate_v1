          ...
          <div className='mx-auto max-w-7xl py-24 px-6 lg:px-8'>
            ...
            <div className='mt-12 space-y-4 sm:mt-16 sm:grid md:grid-cols-3 sm:gap-4 sm:space-y-0'>
              {tiers.map((tier) => (
                <div
                  key={tier.name}
                  className={`${tier.name === "Enterprise" && "bg-gray-800"} divide-y divide-gray-200 rounded-lg border border-gray-200 shadow-sm`}
                >
                  <div className='p-6'>
                    ...
                    <button className='cursor-pointer mt-8 block w-full rounded-md border border-gray-800 bg-indigo-700 py-2 text-center text-sm font-semibold text-white hover:bg-indigo-900'>
                      Buy {tier.name}
                    </button>
                  </div>
